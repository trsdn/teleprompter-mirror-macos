...
- `swift test` is green.
- The change was checked manually with the virtual source display.
...
The app does exactly one thing: capture the private virtual source display,
mirror or rotate the image, and output it full-screen on a physical target
display — with the lowest possible resource usage.
